...
As alluded to in the above, this is because we never enter our render loop again, even if we set the state to be the same
value it was before it will bail out due to equal state and the HTML node will update internally.
