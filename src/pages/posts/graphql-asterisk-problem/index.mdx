...
<aside>
💡 Heck when you use traditional GraphQL as advertised you even lose HTTP caching… ☹️ my issue here is more with the GraphQL-HTTP specification than anything else but still, we need to mention this.
</aside>
...
Maybe it’s all not as bad if you look through it from another lens, from my eyes though, GraphQL requires wartime leadership and not favoring no change. Public perception is in a bad spot and if we favor no change there, well….
