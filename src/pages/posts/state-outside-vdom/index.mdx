...
const useField = (name) => {
  // This function is used to force a re-render, we could also pass this to
  // the FormState class while we register.
...
We might have often heard the sentence "Redux is dead, we can just manage state with context",
however much that latter part might be true it comes with some caveats and should be a decision
