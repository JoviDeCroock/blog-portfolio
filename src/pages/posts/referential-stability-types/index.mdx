...
```ts
import { useCallback, useEffect, useMemo } from 'react-branded-types/react'
```

These are plain re-exports of the hooks, the difference is centered around the type:

```ts
export const useMemo = React.useMemo as <T, const D extends readonly unknown[]>(
  factory: () => T,
  dependencies: CheckedDeps<D>
) => Stable<T>
```

`CheckedDeps<D>` keeps the inferred tuple intact and any unproven values will be an error

```ts
type CheckedDeps<D extends readonly unknown[]> = D & {
  readonly [K in keyof D]: D[K] extends StableDependency ? D[K] : never
}
```

Because this module never pulls in React's permissive overload, there's nothing to fall back to. A bad
dependency is a `never`, and a raw object isn't assignable to `never`:
...
The package still ships the augmentation mode, because some projects genuinely can't touch every import
site and "proofs propagate where they can" beats nothing. But I'd reach for `react-branded-types/react`
in anything new. It's the mode that makes a bad dependency list fail where you wrote it, instead of
...
`createStableContext` moves that requirement onto the provider:
...
```ts
import { useEffect, useMemo, type Stable } from 'react-branded-types/preact'
```
...
component is allowed to rely on this identity". One is an implementation detail that lives in the build,
the other is a contract that lives in the type graph where callers can see it.
...
```ts
useMemo(() => source.filter(rawFilter), [source, rawFilter])
// Type '() => Item[]' is not assignable to type
// 'This dependency is not Stable<T>: memoize it with useMemo/useCallback,
//  source it from useState/useRef, or wrap a module-scope constant with stable().'
```
...
I've been packaging this up as `react-branded-types`. It's an experiment more than a library at this
