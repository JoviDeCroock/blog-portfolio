...
In GraphQL patterns we'll often see computed values symbolised with a `@client` directive.
...
This approach gives us GraphQL fragment-like precision in our updates.
The project's completion rate will automatically update when any todo's completion status changes.
