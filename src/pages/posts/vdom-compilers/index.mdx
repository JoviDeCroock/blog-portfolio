...
we explicitly use them. Let's look at a few of the less obvious ones, in the examples we'll notice that the rerenders
are counted and that we list out the DOM-attribute changes in a list.
...
There are a lot of things we can do to keep our applications fast but as always every paradigm has its limitations and strengths.
