...
be able to drill down into requests and see what is happening as they play out, this is where logs come
into play.
...
multiple calls to REST endpoints/... A good logging setup starts with giving each request
a unique identifier and then using this identifier in every log, this allows us to correlate
logs to a given request.
...
in our resolvers by means of including `ctx.traceId` in the body of the structured log.

Given that every log-message is now associated to a request we can filter/associate them in our favorite logging solution like Datadog, Splunk, ...
Every request can be linked to the resolvers that were called, the requests that resulted and the eventual warning/error you are tracing towards.
...
These solutions allow us to then monitor these metrics and set alerts when they exceed a certain number.
...
Error rates are hard to get right as we need to know whether this was the fault of an external system or service (the error code or message
...
A common solution to this problem is to introduce an error-code pattern where we use the `extensions` property on the `GraphQLError` object
to define the category of our error.
...
Seeing the above error in our logs gives us a quick hint as to where to look as we know we failed in the `checkout` resolver and
our error-code points at our external-provider erroring, we can see where we touch base with the provider and trace that way. We can
now also expand our metrics by grouping on error-path or error-code.

### Clients

Another important aspect of metrics is identifying what kind of client is requesting certain fields, seeing certain errors, ... With GraphQL
we'll often have a lot of consumers of a given endpoint, think about your mobile app that could have a lot of versions on the app store, your
web-app and maybe even some CLI. To know which fields/types we can deprecate we need to be aware of who's using them, to track this we can
add a header or something similar to our requests and include them in the logs.
