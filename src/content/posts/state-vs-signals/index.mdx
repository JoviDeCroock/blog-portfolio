import StateBasedDiagram from '../../../islands/state-vs-signals/StateBasedDiagram'
import SignalBasedDiagram from '../../../islands/state-vs-signals/SignalBasedDiagram'
...
In traditional state management with React hooks, when you call `useState`, any update to that state causes the component—and
...
const Provider = ({ children }) => {
  ...
  return (
    <CountContext.Provider value={{ count, name, setCount, setName }}>
      {children}
...
<aside class="post-callout">
  💡This changes context from a way to distribute state and orchestrate renders
  to only be a form of dependency injection.
</aside>
...
The component hierarchy is irrelevant—what matters is **data dependency**, not **component ancestry**.
...
the parent component, and any other children—unless you've carefully memoized everything.
...
- **Predictable performance**: Re-render locations are determined by where signals are accessed, not by component hierarchy.
  This makes performance predictable and debugging easier—you can trace which components update by following signal reads.
...
The shift from state-based to signal-based rendering is more than a performance optimization—it's a paradigm shift in how we think about reactivity.
Instead of preventing re-renders through memoization, we **only trigger re-renders where they're needed**.

This inversion—from "render where you create state" to "render where you use state"—aligns our code with the actual data flow.
...
As frameworks like Preact Signals and Solid.js demonstrate, this isn't a theoretical improvement—it's a practical one that makes building performant, maintainable applications easier.
